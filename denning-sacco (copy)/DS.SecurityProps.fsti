...
val initiator_comm_key_is_secret:
  ...
  (requires (fun t0 ->
    initiator_state_contains_responder_server_and_comm_key initiator set_state_idx vv initiator_state state_session_idx responder server comm_key /\
...
val responder_comm_key_is_secret:
  ...
  LCrypto unit (pki ds_preds)
  (requires (fun t0 ->
    responder_state_contains_initiator_server_and_comm_key responder set_state_idx vv responder_state state_session_idx initiator server comm_key /\
    set_state_idx < trace_len t0
  ))
  (ensures (fun t0 _ t1 ->
    t0 == t1 /\
    (is_unknown_to_attacker_at (trace_len t0) comm_key \/
    corrupt_id (trace_len t0) (P server) \/ corrupt_id (trace_len t0) (P initiator) \/ corrupt_id (trace_len t0) (P responder) \/
    // TODO: Try to eliminate the following predicate
    is_publishable ds_global_usage (trace_len t0) comm_key)
  ))
