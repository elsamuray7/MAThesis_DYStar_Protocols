...
val initiator_init:
  ...
  server:principal ->
  responder:principal ->
  LCrypto (i:timestamp & secret oyrs_global_usage i (readers [P initiator; P server]) (aead_usage "sk_i_srv")) (pki oyrs_preds)
...
val responder_init:
  responder:principal ->
  server:principal ->
  LCrypto (i:timestamp & secret oyrs_global_usage i (readers [P responder; P server]) (aead_usage "sk_r_srv")) (pki oyrs_preds)
  (requires (fun t0 -> True))
  (ensures (fun t0 (|i,s|) t1 ->
    i == trace_len t0 /\ trace_len t1 > trace_len t0 /\
