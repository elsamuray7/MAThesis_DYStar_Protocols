...
val server_send_msg_2:
  ...
  (ensures fun t0 (mi, si, c_out) t1 -> mi < trace_len t1 /\ si < trace_len t1 /\ trace_len t0 < trace_len t1 /\
                                     clock_get c_out = 1)
...
val responder_recv_msg_3:
  ...
  (requires fun t0 -> msg_idx < trace_len t0)
  (ensures fun t0 (si, c_out) t1 -> si < trace_len t1 /\ trace_len t0 < trace_len t1 /\
